...
RSpec.describe TestTrack::Assignment do
  describe "#variant" do
    ...
    context "when TestTrack is online" do
      before do
        allow(visitor).to receive(:send).with(:tt_offline?).and_return(false)
      end
    ...
    context "when TestTrack is offline" do
      before do
        allow(visitor).to receive(:send).with(:tt_offline?).and_return(true)
      end
