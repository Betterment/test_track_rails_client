...
Migrations to drop splits can be generated by starting the migration name with `retire` or `finish`:

```
rails generate test_track:migration finish_name_of_split
```

```ruby
class FinishNameOfSplit < ActiveRecord::Migration
  def change
    TestTrack.update_config do |c|
      c.finish_split :name_of_split
    end
  end
end
```

The flags `--gate` (`-g`) and `--experiment` (`-e`) are provided as conveniences for setting up splits which are meant to be feature flags (gates), or full fledged experiments:

```
rails generate test_track:migration add_name_of_split --gate
```
...
```
rails generate test_track:migration add_name_of_split --experiment
```
